...
4. `node report/setup-notion.mjs <トークン> <親ページのURL>` を実行 → 「社員稼働レポート」「アプリ別集計」の2つのデータベースが自動で作られる
5. ダッシュボードの「設定」で、共有ドライブのパス・上記2つのデータベースURL・Notionトークンを保存
6. `report/.env`に`ORG_ID`・`INGEST_API_KEY`(ダッシュボードで発行)を設定
7. 「社員」からメンバーを追加し、発行されたセットアップコマンドを各自のMacで実行

Notionのチャートビュー(アプリ別・社員別)だけは自動作成できないため、Notion側で手動追加が必要。
...
ダッシュボードで社員を追加すると、その人専用のセットアップコマンドが発行される(例: `EMPLOYEE_NAME=tanaka ./agent/setup-employee-mac.sh`)。それを本人のMacで1回実行してもらう。

ActivityWatch・Node.jsを自動インストールし、共有ドライブのパス(その人専用の登録があればそれ、無ければ全社共通のデフォルト)をダッシュボードから自動取得し、毎日23:50にログを書き出すジョブを登録する。実行後、システム設定でアクセシビリティ権限の許可が必要。
...
- Notion「社員稼働レポート」: https://app.notion.com/p/5ce380b46fca43acb32c2ee3d9f5d2bb
  - 1人1日=1ページ。一覧で行(社員名)をクリックすると、その日の時間帯ごとのタイムライン(何時に何をしていたか)がページ本文に載っている
